...
function findFirstSingleChar(str) {
  return Array.from(str).filter((c) => Array.from(str).filter((c2) => c2 === c).length === 1)[0];
}
...
function getDigitalRoot(num) {
  let sum = String(num).split('').reduce((a, b) => Number(a) + Number(b), 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < brackets.length;) {
    if (dupl.includes(brackets[i])) {
      dupl = dupl.replace(brackets[i], '');
      i = 0;
    } else i += 1;
  }
...
function getCommonDirectoryPath(pathes) {
  const split = pathes.map((pathItem) => pathItem.split('/'));
  const depth = pathes.map((pathItem) => pathItem.length);
