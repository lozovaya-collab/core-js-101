...
function timeSpanToString(startDate, endDate) {
  ...
  return `${(`${diff.getUTCHours()}`).padStart(2, '0')}:${(`${diff.getMinutes()}`).padStart(2, '0')}:${(`${diff.getSeconds()}`).padStart(2, '0')}.${(`${diff.getMilliseconds()}`).padStart(3, '0')}`;
}
